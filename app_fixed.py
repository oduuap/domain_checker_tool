...
def search_domains_background(keywords, tlds, max_check, min_dr, search_id, mode='keyword', c99_domains=None):
    ...
    try:
        ...
        if mode == 'c99' and c99_domains:
            # C99 Mode: Check HẾT domains từ C99.NL - Tìm TẤT CẢ domains có traffic
            ...
            print(f"✓ Will show ALL domains with DR + Traffic (no limit)")
        ...
        def check_single_domain_c99(domain, idx):
            ...
            try:
                ...
                # Nếu không có DR hoặc không có Traffic, skip
                ...
                # CHỈ LẤY NẾU CÓ DR > 0 VÀ TRAFFIC > 0 ⭐
                if seo_metrics['domain_rating'] > 0 and seo_metrics['organic_traffic'] > 0:
                    # CHECK WHOIS để xem có available để mua không
        ...
        if mode == 'c99':
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                ...
                for future in as_completed(future_to_domain):
                    ...
                    search_progress['message'] = f'Check: {idx}/{len(domains_to_check)} | Có traffic: {len(found_domains)}/{len(domains_to_check)}'
        ...
        found_domains.sort(key=lambda x: (
            not x.get('available', False),  # Available lên đầu (False < True)
            -x['organic_traffic'],          # Traffic cao lên đầu
            -x['domain_rating'],            # DR cao lên đầu
            -x['backlinks']                 # Backlinks cao lên đầu
        ))
        ...
        if mode == 'c99':
            stats_msg = f'''✓ Hoàn thành! Tìm thấy {len(found_domains)} domains có DR

📊 THỐNG KÊ (C99 MODE):
- Đã check: {len(domains_to_check)} domains từ C99.NL
- Có DR: {len(found_domains)} domains ({len(found_domains)*100//len(domains_to_check) if len(domains_to_check) > 0 else 0}%)
- Không có DR: {len(domains_to_check) - len(found_domains)} domains'''
        else:
